...
CREATE TABLE COMMENTBOARD(
	...
	CONSTRAINT FK_ID FOREIGN KEY(USERID) REFERENCES FREEBOARD(USERID)
);
--1번째 댓글
INSERT INTO COMMENTBOARD
VALUES(COMMENTNOSEQ.NEXTVAL, GROUPNOSEQ.NEXTVAL, 1, 0, 1, 'USER', '첫 코멘트입니다.', SYSDATE);
...
VALUES(
		...
		(SELECT COMMENTNO FROM COMMENTBOARD WHERE COMMENTNO = 1),
		(SELECT GROUPSEQ FROM COMMENTBOARD WHERE COMMENTNO = 1) + 1,
		(SELECT TITLETAB +1 FROM COMMENTBOARD WHERE COMMENTNO = 1),
		1,
		'USER2',
		'무플방지',
		SYSDATE
